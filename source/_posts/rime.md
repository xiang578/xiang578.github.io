...
![](http://media.xiang578.com/15599183665365.jpg)

## 引言

如何将汉字输入到计算机中是一个有趣的编码问题，目前市面上主流的方案主要分成音码、形码、音形码等多种输入方案。和大多数人一样，之前我一直使用全拼，而且得益于当前的 NLP 技术的发展，使用诸如搜狗输入法等拥有云词库的输入方案，输入效率可以媲美五笔输入法。

但是今天要和大家分享的是，我从今年年初开始使用的全新音码输入方案——小鹤双拼。最初接触到双拼是在李笑来的《把时间当作朋友》：
...
李笑来思考的问题正是如何利用最小的代价快速的输入文字。这里代价包括两个方面，输入方案的学习成本以及输入文字的速度。简单分析一下，主流输入法需要用户在键盘上敲击一些字符，然后映射汉字。就是说输入文字的速度和两个因数有关：敲击键盘的次数（对应计算机中的码长）以及重码的字符数量（对于拼音输入法来说，这里指的是每个拼音对应多少的字）。

从这两个指标来说，五笔应该是接近输入法的极限，五下敲击键盘肯定能选定你需要的汉字。但是学习五笔需要记忆大量的说是有序其实没有太多规律的字根，学习曲线不是一般的陡峭。
...
![](http://media.xiang578.com/15600908556759.jpg)
